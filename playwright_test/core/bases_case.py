...
class BaseBrowser:
    ...
    浏览器的创建
    """
    def __init__(self, config_env, browser=None, context=None, page=None):
        self.pages = {}
        if all([config_env, browser, context, page]):
            print("创建浏览器对象")
    ...
    def open_browser(self, browser_type):
        ...
        try:
            ...
            self.browser, self.context, self.page = BaseBrowser.create_browser(browser_type,
...
class PageMixin(BaseBrowser):
    ...
    def save_page_img(self, name, path=''):
        ...
        try:
            if not path:
                path = self.error_pic_path
            print(f"正在保存页面截图，截图名称：{name}，截图保存路径：{path}")
